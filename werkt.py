...
# UI Basisscherm:
root = Tk()
root.configure(height=250, width=430, background=background)
root.title('NS Automaat')


# Knoppen in UI
Label(root, text='Welkom bij NS', font=top_text,
      foreground=front_text, background=back_text).place(x=100, y=30)

actuele = Button(root, text='Actuele \n reisinformatie', foreground=front_knop,
                 background=back_knop, font=knop, command=new_winF, height=2, width=15).place(x=60, y=100)

station = Button(root, text='Station \n informatie', foreground=front_knop,
                 background=back_knop, font=knop, command=inputprompt, height=2, width=15).place(x=200, y=100)
namen = Label(root, text='Door: Abuzar, Fong, Bunjamin en Youssef',
              foreground=front_text, background=back_text).place(x=100, y=180)
